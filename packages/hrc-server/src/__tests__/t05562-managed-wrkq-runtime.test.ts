...
describe('T-05562 managed broker wrkq authority', () => {
  ...
  for (const claimed of [false, true]) {
    test(`managed broker projects locator/token authority when ${
      claimed ? 'claimed' : 'unclaimed'
    } and claim authority only when claimed`, async () => {
      const directory = await mkdtemp(
        join(tmpdir(), `hrc-t05562-${claimed ? 'claimed' : 'unclaimed'}-`)
      )
      tempDirs.push(directory)
      const db = openHrcDatabase(join(directory, 'state.sqlite'))
      try {
        const hostSessionId = `hsid-managed-${claimed ? 'claimed' : 'unclaimed'}`
        const runtimeId = `rt-managed-${claimed ? 'claimed' : 'unclaimed'}`
