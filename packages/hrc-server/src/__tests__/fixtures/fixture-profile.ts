 ...
 * (core/config/agent-profile-toml.js at ASP 0.1.1-dev.20260917231122), which
 * itself parses with `@iarna/toml` — this fixture uses the same parser, so
 * TOML syntax acceptance is identical and only the semantic validation is
 * narrowed. Production code never touches this file: the daemon reads profile
 * facts from the aspd declaration observation.
 *
