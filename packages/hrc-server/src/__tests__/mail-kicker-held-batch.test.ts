...
import { dropAckedHeldMember, holdQueueForBusyTarget, revalidateHeldBatch } from 'hrc-mail-kicker'
...
describe('mail-kicker held-batch seam', () => {
  ...
  it('drops terminal members at freeze time and treats ack as a pure local dequeue', async () => {
    ...
    expect((await revalidateHeldBatch(kicker, held)).map((item) => item.envelope.id)).toEqual([
      survivor.id,
    ])
    expect(internals.db.mailDrives.presentationEnvelopeIds(held.driveAttemptId)).toEqual([
      survivor.id,
    ])
    expect(dropAckedHeldMember(kicker, survivor.id, 'envelope_acked_before_injection')).toBe(true)
