...
/** The turn text that used to be printed beside a stranded obligation. */
...
describe('T-07964 §3 kicker.unowned_turn', () => {
  it('fires when a turn ends on a runtime still holding a presented envelope', async () => {
    ...
    const [unowned] = lines('wrkq.kicker.unowned_turn')
  ...
  it('names the stranded envelope and NOT what the turn said', async () => {
    // T-08093 inverted this from "names the canonical response the stranded turn
    // would have replied". The line exists to name an obligation nobody is
    // holding; printing the turn's text beside it asserted that the text was a
    // candidate answer, which is exactly the claim HRC no longer makes.
    seedRuntime('failed')
    seedClaimedDrive()
    db.mailDrives.failWithoutStart(DRIVE, 'compiler priming wait aborted')
    ...
    const [unowned] = lines('wrkq.kicker.unowned_turn')
    expect(unowned?.detail).toMatchObject({ envelopeIds: [ENVELOPE] })
    expect(unowned?.detail).not.toHaveProperty('canonicalResponse')
    expect(JSON.stringify(unowned?.detail)).not.toContain(CANONICAL_RESPONSE)
  })
  ...
  it('stays silent when a live attempt owns the turn', async () => {
    ...
    expect(lines('wrkq.kicker.unowned_turn')).toHaveLength(0)
  })
