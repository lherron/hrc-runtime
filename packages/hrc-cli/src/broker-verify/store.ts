import { Database } from 'bun:sqlite'

import { resolveDatabasePath } from 'hrc-core'

import { safeJsonParse } from './json.js'
import type {
  BrokerEventRow,
  BrokerInvocation,
  BrokerVerifyCandidate,
  HrcLifecycleRow,
  RawMirrorRow,
} from './types.js'

type CandidateRow = {
  invocation_id: string
  scope_ref: string
  lane_ref: string
  runtime_id: string
  run_id: string | null
  host_session_id: string
  generation: number
  broker_driver: string
  broker_protocol: string
  invocation_state: string
  created_at: string
  updated_at: string
  event_count: number
  first_seq: number | null
  last_seq: number | null
  first_event_at: string | null
  last_event_at: string | null
}

type InvocationRow = {
  invocation_id: string
  operation_id: string
  runtime_id: string
  run_id: string | null
  broker_driver: string
  broker_protocol: string
  invocation_state: string
  current_harness_generation: number | null
  current_turn_attempt: number | null
  created_at: string
  updated_at: string
}

type EventRow = {
  invocation_id: string
  seq: number
  time: string
  type: string
  run_id: string | null
  runtime_id: string
  harness_generation: number | null
  turn_attempt: number | null
  broker_event_json: string
  hrc_event_seq: number | null
  projection_status: string
  projection_error: string | null
  created_at: string
}

type RawRow = {
  seq: number
  source: string
  event_kind: string
  event_json: string
}

type LifecycleRow = {
  hrc_seq: number
  event_kind: string
  payload_json: string
}

export class BrokerVerifyStore {
  readonly db: Database

  constructor(path = resolveDatabasePath()) {
    this.db = new Database(path, { readonly: true, create: false })
  }

  close(): void {
    this.db.close()
  }

  listCandidates(scopeRef: string): BrokerVerifyCandidate[] {
    const rows = this.db
      .query<CandidateRow, [string]>(
        `
          SELECT
            bi.invocation_id,
            r.scope_ref,
            r.lane_ref,
            bi.runtime_id,
            bi.run_id,
            r.host_session_id,
            r.generation,
            bi.broker_driver,
            bi.broker_protocol,
            bi.invocation_state,
            bi.created_at,
            bi.updated_at,
            COUNT(bie.seq) AS event_count,
            MIN(bie.seq) AS first_seq,
            MAX(bie.seq) AS last_seq,
            MIN(bie.time) AS first_event_at,
            MAX(bie.time) AS last_event_at
          FROM broker_invocations bi
          JOIN runtimes r ON r.runtime_id = bi.runtime_id
          LEFT JOIN broker_invocation_events bie ON bie.invocation_id = bi.invocation_id
          WHERE r.scope_ref = ?
          GROUP BY bi.invocation_id
          ORDER BY bi.updated_at DESC, bi.created_at DESC
          LIMIT 50
        `
      )
      .all(scopeRef)

    return rows.map((row) => ({
      invocationId: row.invocation_id,
      scopeRef: row.scope_ref,
      laneRef: row.lane_ref,
      runtimeId: row.runtime_id,
      ...(row.run_id !== null ? { runId: row.run_id } : {}),
      hostSessionId: row.host_session_id,
      generation: row.generation,
      brokerDriver: row.broker_driver,
      brokerProtocol: row.broker_protocol,
      invocationState: row.invocation_state,
      createdAt: row.created_at,
      updatedAt: row.updated_at,
      eventCount: row.event_count,
      ...(row.first_seq !== null ? { firstSeq: row.first_seq } : {}),
      ...(row.last_seq !== null ? { lastSeq: row.last_seq } : {}),
      ...(row.first_event_at !== null ? { firstEventAt: row.first_event_at } : {}),
      ...(row.last_event_at !== null ? { lastEventAt: row.last_event_at } : {}),
      confidence: 'exact-scope',
    }))
  }

  getInvocation(invocationId: string): BrokerInvocation | undefined {
    const row = this.db
      .query<InvocationRow, [string]>(
        `
          SELECT
            invocation_id,
            operation_id,
            runtime_id,
            run_id,
            broker_driver,
            broker_protocol,
            invocation_state,
            current_harness_generation,
            current_turn_attempt,
            created_at,
            updated_at
          FROM broker_invocations
          WHERE invocation_id = ?
        `
      )
      .get(invocationId)
    if (!row) return undefined
    return {
      invocationId: row.invocation_id,
      operationId: row.operation_id,
      runtimeId: row.runtime_id,
      ...(row.run_id !== null ? { runId: row.run_id } : {}),
      brokerDriver: row.broker_driver,
      brokerProtocol: row.broker_protocol,
      invocationState: row.invocation_state,
      ...(row.current_harness_generation !== null
        ? { currentHarnessGeneration: row.current_harness_generation }
        : {}),
      ...(row.current_turn_attempt !== null ? { currentTurnAttempt: row.current_turn_attempt } : {}),
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    }
  }

  listBrokerEvents(invocationId: string): BrokerEventRow[] {
    const rows = this.db
      .query<EventRow, [string]>(
        `
          SELECT
            invocation_id,
            seq,
            time,
            type,
            run_id,
            runtime_id,
            harness_generation,
            turn_attempt,
            broker_event_json,
            hrc_event_seq,
            projection_status,
            projection_error,
            created_at
          FROM broker_invocation_events
          WHERE invocation_id = ?
          ORDER BY seq ASC
        `
      )
      .all(invocationId)
    return rows.map((row) => ({
      invocationId: row.invocation_id,
      seq: row.seq,
      time: row.time,
      type: row.type,
      ...(row.run_id !== null ? { runId: row.run_id } : {}),
      runtimeId: row.runtime_id,
      ...(row.harness_generation !== null ? { harnessGeneration: row.harness_generation } : {}),
      ...(row.turn_attempt !== null ? { turnAttempt: row.turn_attempt } : {}),
      brokerEventJson: safeJsonParse(row.broker_event_json),
      brokerEventJsonText: row.broker_event_json,
      ...(row.hrc_event_seq !== null ? { hrcEventSeq: row.hrc_event_seq } : {}),
      projectionStatus: row.projection_status,
      ...(row.projection_error !== null ? { projectionError: row.projection_error } : {}),
      createdAt: row.created_at,
    }))
  }

  getRawMirror(seq: number): RawMirrorRow | undefined {
    const row = this.db
      .query<RawRow, [number]>(
        `SELECT seq, source, event_kind, event_json FROM events WHERE seq = ?`
      )
      .get(seq)
    if (!row) return undefined
    return {
      seq: row.seq,
      source: row.source,
      eventKind: row.event_kind,
      eventJson: safeJsonParse(row.event_json),
      eventJsonText: row.event_json,
    }
  }

  findLifecycle(input: {
    runtimeId: string
    runId?: string | undefined
    generation?: number | undefined
    eventKind: string
  }): HrcLifecycleRow | undefined {
    const runPredicate = input.runId !== undefined ? 'AND run_id = ?' : ''
    const generationPredicate = input.generation !== undefined ? 'AND generation = ?' : ''
    const args: Array<string | number> = [input.runtimeId, input.eventKind]
    if (input.runId !== undefined) args.push(input.runId)
    if (input.generation !== undefined) args.push(input.generation)
    const row = this.db
      .query<LifecycleRow, Array<string | number>>(
        `
          SELECT hrc_seq, event_kind, payload_json
          FROM hrc_events
          WHERE runtime_id = ?
            AND event_kind = ?
            ${runPredicate}
            ${generationPredicate}
          ORDER BY hrc_seq ASC
          LIMIT 1
        `
      )
      .get(...args)
    if (!row) return undefined
    return {
      hrcSeq: row.hrc_seq,
      eventKind: row.event_kind,
      payload: safeJsonParse(row.payload_json),
    }
  }
}
