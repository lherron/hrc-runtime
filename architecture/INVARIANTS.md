...
Mobile suffix-roster families are finite: each allowed base and every member produced by HRC's fixed ordered suffix list has an exact placement task-default naming the same concrete home node before the surface is enabled. Implicit suffix starts are routed by origin HRC from policy and registry state; ACP supplies no node assertion. A remote home is invoked only through the authenticated, capability-advertised rosterStart peer verb. The receiver independently preflights retirement, authority and binding state, exact local policy, and materialization capability for the whole family before mutation, localizes the runtime intent, and then owns the existing atomic roster claim/start and durable idempotency record. Operator CLI starts remain explicit_local. Retryable peer failure is synchronous runtime_unavailable and does not use the message outbox.
