...
Status: DRAFT rev 5 — material amendment to APPROVED rev 3 (DM #21133, law `hrc-runtime.viewer-presentation-sidecar`), submitted to daedalus. Trigger: P3 live shadow run reaped production panes (§10).  
Date: 2026-08-26  
...
- **Legacy window adoption.** A window carrying `hrc_role=headless-sessions-window` without `hrc_owner_root` may be claimed by the first actor (viewer or, until P4, in-daemon path) to stamp `hrc_owner_root` on it; ghostmux's find-or-create makes the claim atomic. Legacy panes inside a claimed window keep their own legacy status until adopted per §4.5 — claiming a window claims none of its panes. A legacy pane adopted per §4.5 stays where it is (in place); new panes for that runtime's tab key go to the owner's window, so a tab key may temporarily span an adopted legacy window and the owner's window — transitional, bounded by those runtimes' lifetimes.
...
- The pane key is scoped by owner: find-or-create matches on `(hrc_owner_root, hrc_pane_key)`. Two daemons hosting the same `(scopeRef, laneRef)` (prod and hrc-dev both running `cody@hrc-runtime:T-07595`) get two panes in two different windows; the in-daemon path today would have collided on that key.
