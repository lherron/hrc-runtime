...
Status: APPROVED rev 3 — daedalus ruling DM #21133 (2026-08-26); durable law `hrc-runtime.viewer-presentation-sidecar` (bc10bbdc). Rev 3a (2026-08-26): operator ruling by Lance added to §2.2 and §6; no design change (see §10).  
Date: 2026-08-26  
Author: mable@hrc-runtime  
Primary systems: hrc-server (event ledger, runtime lifecycle), `agent-control-plane/packages/hrc-viewer`, `ghostmux` CLI, ScriptableGhostty
...
- **Exactly one viewer per user, bound to the production daemon (operator ruling, Lance, 2026-08-26).** Ghostty's metadata registry is per user and shared by every hrc daemon of that user; this design assumes a single viewer-bearing daemon per user and does not fence panes by owning daemon. Non-production daemons (`hrc-dev` and any other lane) run with `HRC_GHOSTTY_VIEWERS=0` until Phase 4 and never get a viewer. Multi-daemon viewer isolation is out of scope by ruling: the rev 4–6 amendment that attempted it is withdrawn.
...
- Bun/TypeScript, `agent-control-plane/packages/hrc-viewer`, consuming the ACP-pinned `@praesidium/hrc-sdk` (`HrcClient.watchBoundedEvents`, `tailEvents`, `listLatestEventBySession`, `listPresentationRuntimes` — the §5.4 set and nothing else) and shelling to `ghostmux`.
...
`GET /v1/presentation/runtimes` returns, for every runtime whose status is non-terminal, one row: identity (`runtimeId`, `hostSessionId`, `scopeRef`, `laneRef`, `generation`, `status`), `presentation` (the persisted record, or absent for pre-Phase-2 generations), `tmux`, `title`, and `operatorSurfaces`. `operatorSurfaces` contains only active `ghostty` bindings whose interactive attach captured a nonempty controlling-terminal `clientTty`; legacy or unqualified bindings are not eligible. A new Ghostty bind retires an older active binding for that `clientTty` in the same database transaction. No invocation-local field appears here; the read model carries only durable facts. It is a **projection over the store only**: it reads runtime rows, the persisted §5.1 record, hosting/tmux state, `session_titles`, and `surface_bindings`, and returns. It does **not** call `reconcileTmuxRuntimeLiveness`, probe tmux, attach, or append events. SDK: `HrcClient.listPresentationRuntimes()`.
...
- the viewer package's SDK usage is limited to the corresponding client methods, checked by ACP's `check-boundaries` gate.
...
| **3. Build hrc-viewer; shadow-validate** | New package + LaunchAgent; `ghostmux.ts` viewer portion, `headless-viewer-status.ts`, theme/prefix/label modules and their tests move across. Shadow runs against the **production** daemon on max3 (the only viewer-bearing daemon, per §2.2 ruling): the sidecar runs against production and Phases 3 and 4 land as one motion (operator ruling, Lance, 2026-08-26: forward fix only — no backward compatibility, no feature flag). `HRC_GHOSTTY_VIEWERS` / `HRC_GHOSTTY_VIEWER_LINGER_SECONDS` are deleted from code, not toggled; there is no rollback-by-flag. The codex tmux-tui presentation decision is decoupled from the viewer flag as the first step (pre-pulled from Phase 4). | For each of: `hrc start` (headless codex + claude), `hrc run` (viewer must be **suppressed**, including after a viewer restart + reconcile while the run is live), then a detached `hrc start` reusing that runtime (viewer **must appear**), `wrkc say` to a cold scope, two role-qualified scopes for one agent/task (two panes), `hrc session retitle`, `hrc runtime terminate` (pane survives `linger` then dies), daemon graceful restart, viewer restart, Ghostty quit/reopen — the resulting `ghostmux list-windows/list-surfaces --json` topology is identical to what the in-daemon path produced, screenshots attached to the task. Additionally: with the viewer running against a runtime whose tmux socket has been removed, the runtime's status does not change until hrc's own sweep runs. Row 8 uses `HRC_VIEWER_LINGER_SECONDS=2` on the sidecar only. Pre-existing production panes (in-daemon, pre-record generations) must be adopted, never duplicated or reaped while their runtime is live. |
...
Rollback is forward-only by ruling: fix the viewer. (Earlier text describing a flag-based rollback for phases ≤ 3 is superseded.)
...
The HRCMac campaign (T-07334..37) embeds libghostty and attaches to broker-tmux directly; its spec names "viewer sunset" as a deliberately separate future spec. This extraction is that sunset's on-ramp: once B is `agent-control-plane/packages/hrc-viewer`, sunset is the removal of one package and one LaunchAgent, with no further daemon surgery. HRCMac could also become a second consumer of the same `presentation` event fields (§5.1) with no additional hrc work.
...
**rev 4–6 WITHDRAWN** (2026-08-26). The P3 shadow run was pointed at `hrc-dev` while production still ran the in-daemon viewer; two daemons on one Ghostty registry let the dev sidecar reap three production panes (T-07595 evidence). Three amendment revisions attempted per-daemon pane ownership; daedalus rejected each on a further unfenced path (placement/split/equalize, non-atomic legacy-window claim, dry-run unable to read metadata). Lance ruled instead: one viewer per user, bound to the production daemon; non-production daemons never run a viewer. The approved rev 3 design is unchanged; §2.2 and the Phase 3 gate record the ruling.

**Operator ruling 2026-08-26 (Lance): forward fix only** — no backward compatibility, no feature flag. Phases 3 and 4 collapse into one motion; the flag-OFF shadow window is dropped.
