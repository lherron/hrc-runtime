...
| 2026-07-21 | T-06633 | Moving authority is safest as one shared-lock sequence with explicit nowhere states: revoke the exact old row, CAS only after remote proof, then require exact local activation. | check | `t06633-fenced-rebind.test.ts` |
| 2026-07-27 | T-07013 | Supervision stays automatable when every terminal event classifies its coarse outcome, preserves the precise result, and carries the same exit code the process returns. | check | `monitor-condition-engine.test.ts`, `t06575-monitor-until-red.test.ts` |
