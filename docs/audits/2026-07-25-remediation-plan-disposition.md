...
## Campaign execution record (2026-07-25 afternoon, Mable supervising cody + clod)

All five actionable records implemented same-day by cody@hrc-runtime:minisvc (T-06971, T-06972) and clod@hrc-runtime:minisvc (T-06970, T-06973, T-06974 after rebalance), each in its own worktree, each landed on `origin/main` with a green solo `just verify` under a host-wide one-bar-at-a-time mutex (adopted after two concurrent full bars starved hrc-server's fixture daemons and produced a spurious "pre-existing repo defect" diagnosis, withdrawn on solo re-run). Landings, in order: `e022336` idempotency (+`edbaae2` worktree fixture repair), `eb2ce97` selectors, `4953c8b` history columns, `ea1dd64` federation timeout, `2b6a681` small-fix batch. T-06975 remains draft by design.

Consolidated deploy + live e2e on svc: installed `release-20260725152251850-23006` @ `2b6a681` (first cut was stale at `e022336` — canonical checkout hadn't been pulled; caught via the publish proof's sourceCommit), restarted via launchd, `runningEqualsInstalled=true`, migration 0035 backfilled 18,050 live rows with zero nulls. Live smokes all green, evidence on each task: idempotency key replayed to the same run across a real daemon restart; six-cell selector round-trip on a real DM; history latency 0.38–0.96s → 0.09s flat with zero missing rows under SQL filtering; lease-GC `changed:false` no-op tick; janitor pruned 5 spent campaign worktrees while the detached baseline survived a `--yes` run via the new deliberate-skip path; both stale strings corrected. Boundary: T-06972's cross-node sleeping-peer scenario was not exercised live — its proof is the landing suite + loopback corpus.

Two platform observations retained: (1) spaces-config infers project id from a linked worktree's basename and fabricates a nested path under an ambient ASP envelope (recorded on T-06971; agent-spaces follow-up candidate); (2) bun-based tools run from the canonical checkout auto-load `.env.local` (`WRKQD_TOKEN=dev`), which silently 401s their wrkq children against remote workrpc — operational landmine, worked around by running outside the checkout. lab/max3 pick up this release on their next routine install.
